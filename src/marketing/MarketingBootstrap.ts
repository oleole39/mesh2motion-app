...
export class MarketingBootstrap {
  ...
  public setup_model_buttons (): void {
    ...
    human_button?.addEventListener('click', () => {
      this.mesh2motion_engine.load_model_step.load_model_file(baseUrlNoTrailingSlash + '/models/model-human.glb', 'glb')
      this.skeleton_type = SkeletonType.Human
      this.change_active_skeleton(human_button)
    })

    fox_button?.addEventListener('click', () => {
      this.mesh2motion_engine.load_model_step.load_model_file(baseUrlNoTrailingSlash + '/models/model-fox.glb', 'glb')
      this.skeleton_type = SkeletonType.Quadraped
      this.change_active_skeleton(fox_button)
    })

    bird_button?.addEventListener('click', () => {
      this.mesh2motion_engine.load_model_step.load_model_file(baseUrlNoTrailingSlash + '/models/model-bird.glb', 'glb')
      this.skeleton_type = SkeletonType.Bird
      this.change_active_skeleton(bird_button)
    })

    dragon_button?.addEventListener('click', () => {
      this.mesh2motion_engine.load_model_step.load_model_file(baseUrlNoTrailingSlash + '/models/model-dragon.glb', 'glb')
  ...
  public add_event_listeners (): void {
    ...
    this.mesh2motion_engine.load_model_step.addEventListener('modelLoaded', () => {
      ...
      this.mesh2motion_engine.load_skeleton_step.load_skeleton_file(baseUrlNoTrailingSlash + '/' + this.skeleton_type)
    ...
    this.mesh2motion_engine.load_skeleton_step.addEventListener('skeletonLoaded', () => {
      this.mesh2motion_engine.animations_listing_step.set_animations_file_path(baseUrlNoTrailingSlash + '/animations/')
